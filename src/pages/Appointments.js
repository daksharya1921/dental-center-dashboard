...
// Full improved Appointments component with role check, file preview modal, and filtering
// import { useState, useEffect } from 'react';
// import { Link, Navigate } from 'react-router-dom';
// import {
//   Box, Typography, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper,
//   IconButton, Chip, Tooltip, Dialog, DialogTitle, DialogContent, DialogActions, Fade, Stack, Avatar,
//   Card, CardContent, Grid, Divider, TextField, InputAdornment, Badge, Snackbar, Alert, MenuItem, Select
// } from '@mui/material';
// import {
//   Delete as DeleteIcon, Edit as EditIcon, CalendarToday as CalendarIcon, AccessTime as TimeIcon,
//   Person as PersonIcon, AttachMoney as MoneyIcon, EventAvailable as EventIcon, Search as SearchIcon,
//   FilterList as FilterIcon, Schedule as ScheduleIcon, CheckCircle as CompletedIcon, Cancel as CancelledIcon
// } from '@mui/icons-material';
// import { getIncidents, deleteIncident, getPatients } from '../utils/dataService';
// import { format } from 'date-fns';
...
//     });
//     setFilteredIncidents(filtered);
//   }, [searchTerm, selectedStatus, incidents]);
...
//   const getPatientInitials = (id) => getPatientName(id).split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);

//   const formatDate = (date) => format(new Date(date), 'MMM dd, yyyy');
//   const formatTime = (date) => format(new Date(date), 'hh:mm a');

//   const getStatusIcon = (s) => s === 'Completed' ? <CompletedIcon fontSize="small" /> :
//     s === 'Cancelled' ? <CancelledIcon fontSize="small" /> : <ScheduleIcon fontSize="small" />;
//   const getStatusColor = (s) => s === 'Completed' ? 'success' : s === 'Cancelled' ? 'error' : 'warning';

//   const handleCloseSnackbar = () => setShowSnackbar(false);

//   const handlePreview = (files) => setPreviewFiles(files);
...
//           <Select fullWidth value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)}>
...
//                 <TableCell>{getPatientName(row.patientId)}</TableCell>
//                 <TableCell>{formatDate(row.appointmentDate)} {formatTime(row.appointmentDate)}</TableCell>
//                 <TableCell>
//                   <Chip icon={getStatusIcon(row.status)} label={row.status} color={getStatusColor(row.status)} />
//                 </TableCell>
//                 <TableCell>
//                   <Button onClick={() => handlePreview(row.files)}>View</Button>
//                 </TableCell>
//                 <TableCell>
//                   <IconButton component={Link} to={`/appointments/edit/${row.id}`}><EditIcon /></IconButton>
//                   <IconButton onClick={() => setConfirmId(row.id)}><DeleteIcon color="error" /></IconButton>
//                 </TableCell>
//               </TableRow>
//             ))}
//           </TableBody>
...
//           <Button onClick={() => handleDelete(confirmId)} color="error">Delete</Button>
//         </DialogActions>
//       </Dialog>

//       {/* File Preview Modal */}
//       <Dialog open={previewFiles.length > 0} onClose={() => setPreviewFiles([])} maxWidth="md" fullWidth>
//         <DialogTitle>Attached Files</DialogTitle>
//         <DialogContent dividers>
//           {previewFiles.map((file, i) => (
//             <Box key={i} mb={2}>
//               <Typography variant="body2">{file.name}</Typography>
//               {file.url.startsWith('data:image') && <img src={file.url} alt={file.name} style={{ maxWidth: '100%' }} />}
//               {file.url.startsWith('data:application/pdf') && <iframe src={file.url} width="100%" height="400px" title={file.name} />}
//             </Box>
//           ))}
//         </DialogContent>
//         <DialogActions>
//           <Button onClick={() => setPreviewFiles([])}>Close</Button>
//         </DialogActions>
//       </Dialog>

//       {/* Snackbar */}
//       <Snackbar open={showSnackbar} autoHideDuration={3000} onClose={handleCloseSnackbar}>
//         <Alert severity="success" onClose={handleCloseSnackbar}>Appointment deleted</Alert>
//       </Snackbar>
//     </Box>
//   );
// };
